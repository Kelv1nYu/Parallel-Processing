	...
	__global float* SCRes)  //返回结果
{
	...
	if (fHitPointNorm > *ProbeRadiusPixel - cuEPSILON && fHitPointNorm < *SectorRadiusPiexl + cuEPSILON) {
		...
		if (fSamplePointAngle > *StartAngle - cuEPSILON && fSamplePointAngle < *EndAngle + cuEPSILON) {
			...
			float res = 0.0;
			// 若转换后该点区间在范围内
			if (PosXint1 >= 2 && PosXint1 < *iLine - 3 && PosYint1 >= 2 && PosYint1 < *iSmapleNum - 3) {
				for (int i = -2; i <= 3; i++) {
					for (int j = -2; j <= 3; j++) {
						res += Lanczos3Kernel(PosX - (PosXint1 + i)) * Lanczos3Kernel(PosY - (PosYint1 + j)) * (data)[(PosXint1 + i) * (*iSmapleNum) + (PosYint1 + j)];
					}
				}
